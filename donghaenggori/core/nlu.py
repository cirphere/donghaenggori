...
# 진료과 이름으로 끝나는 상호 — 동네 의원은 '○○정형외과' 가 '○○의원' 보다
# 흔하다. 실제 대본에서 "병원은 한마음정형외과" 를 놓쳤고, 그러면 그 자리를
# 과거 이력이 채워 **어르신이 말한 병원이 사라진다.**
#
# **위 규칙에 섞지 않고 따로 둔다.** 꼬리 목록에 넣었더니 정규식이 욕심을 내
# "담양제일병원 내과" 를 '담양제일병원'+'내과' 로 잡았고, 띄어쓰기 규칙에
# 걸려 병원이 통째로 사라졌다. 있던 것을 잃는 쪽이 새로 얻는 것보다 나쁘다.
#
# **붙여 쓴 것만 본다(\s* 없음).** "무릎 아파서 정형외과 갈라고" 는 띄어
# 썼으니 걸리지 않는다.
_DEPT_SUFFIX = ("정형외과", "이비인후과", "재활의학과", "산부인과", "비뇨기과",
                "정신건강의학과", "가정의학과", "신경외과", "성형외과", "안과",
                "치과", "내과", "외과", "신경과", "피부과", "소아과")
# 띄어쓰기를 허용한다 — "한마음 정형외과" 처럼 띄어 쓰는 것이 오히려 흔하다.
# 붙여 쓴 것만 보다가 시연 대본의 병원을 놓쳤다. 대신 앞말이 용언이면
# 거른다("무릎이 아파서 정형외과") — 아래 detect_hospital 참조.
# 이름은 **두 어절까지** 받는다. "밝은 눈 안과" 처럼 상호를 띄어 쓰는 일이
# 흔한데 한 어절만 보면 '눈안과' 가 된다 — 어르신이 말한 병원이 반쪽만 남는다.
#
# 두 어절을 허용하면 "무릎이 아파서 정형외과" 도 걸리지만, 아래에서 어절마다
# 차단 목록과 용언 어미를 본다.
_CLINIC_RE = re.compile(
    r"([가-힣A-Za-z0-9]+(?:\s+[가-힣A-Za-z0-9]+)?)(\s*)("
    + "|".join(sorted(_DEPT_SUFFIX, key=len, reverse=True)) + r")")
...
_NOT_A_NAME = {"그", "저", "저번", "지난번", "예전", "전", "전에", "갔던", "간", "봐준",
               ...
               # '새로 생긴 실버정형외과' 가 '생긴실버정형외과' 가 됐다.
               # 위의 '갔던·다니던' 과 같은 관형형인데 빠져 있었다.
               "생긴", "연", "문", "열은", "있는", "있던", "하는", "가는",
...
_VERB_ENDINGS = ("서", "고", "러", "며", "게", "다가", "는데", "니까", "지만",
                 "도록", "에", "까지", "부터", "처럼", "려고", "면서",
                 # '싶은데 정형외과' 가 '싶은데정형외과' 라는 병원이 됐다.
                 # '는데' 만 있어서 '은데·인데' 가 빠졌다. 상호가 이 어미로
                 # 끝나는 경우는 없다.
                 "은데", "인데", "라서", "느라", "다고", "든지",
                 # "영 안 보여 안과 가봐야" 가 '보여안과' 라는 병원이 됐다.
                 # 띄어 쓴 진료과 상호를 받기 시작하면서(#127) 앞말이 용언인
                 # 경우가 훨씬 넓어졌다 — 종결·연결 어미를 더 채운다.
                 "여", "워", "해", "봐", "와", "야", "지", "니", "면",
                 # "아파가지고 정형외과" — 전라도에서 아주 흔한 연결어미다.
                 "가지고", "갖고", "쓰고", "노라고")
...
def detect_hospital(text: str) -> str | None:
    ...
    if not found:
        ...
        # 때만. 순서를 뒤집으면 "담양제일병원 내과" 같은 발화에서 이미 잘
        # 잡히던 병원을 잃는다.
        for m in _CLINIC_RE.finditer(text):
            name, gap, suffix = m.group(1), m.group(2), m.group(3)
            if name in _NOT_A_NAME:
                continue
            # **띄어 썼으면 앞말이 상호인지 본다.** "한마음 정형외과" 는
            # 상호이고 "무릎이 아파서 정형외과" 는 아니다. 둘을 가르는 것은
            # 앞말이 용언 어미로 끝나느냐다 — 병원 꼬리에 쓰는 규칙과 같다.
            # 어절마다 본다 — 두 어절을 받으니 앞 어절도 검사해야 한다.
            # "어제 간 안과" 의 '어제', "무릎이 아파서 정형외과" 의 '아파서'.
            # **앞 어절을 버리고 남은 것을 쓴다.** 통째로 버리면 있던 병원을
            # 잃는다 — "낼 한마음정형외과" 가 그래서 통째로 사라졌고, 그 자리를
            # 과거 이력이 채웠다. 상호 앞에 시간 표현·관형형이 붙는 것은 흔하다.
            #
            # **gap 이 비어도 검사한다.** 붙여 쓴 경우(실버정형외과)에는 앞 어절
            # 검사를 건너뛰고 있었다 — "새로 생긴 실버정형외과" 가
            # '생긴실버정형외과' 라는 없는 병원이 됐다(HO-7). 띄어 썼는지는
            # 상호 판단과 무관하다.
            # #138 이 "앞 어절이 걸리면 그 어절만 버린다" 를 넣었다. 그 방향이
            # 맞는데 조건이 좁았다 — 두 어절이고 앞말이 차단 목록일 때만이라,
            # 앞말이 용언으로 끝나는 경우(생긴·있는)는 그대로 통과했다.
            # 앞에서부터 걸리는 만큼 버린다.
            words = name.split()
            while words and (words[0] in _NOT_A_NAME
                             or (len(words) > 1 and words[0].endswith(_VERB_ENDINGS))):
                words.pop(0)
            if not words:
                continue
            # 남은 한 글자로 상호를 만들지는 않는다(#138) — "어제 간 안과" 의 '간'.
            if len(words[0]) < 2:
                continue
            # 남은 것이 용언으로 끝나면 상호가 아니다 — "무릎이 아파서 정형외과".
            if words[-1].endswith(_VERB_ENDINGS) or words[-1] in _NOT_A_NAME:
                continue
            # **1차가 이미 보고 버린 것을 줍지 않는다.** 앞말에 병원 꼬리가
            # 들어 있으면 그것은 1차 규칙의 대상이었다 — "백병원에는 피부과가
            # 없으니" 에서 1차는 부정 문맥이라 '백병원' 을 버렸는데, 2차가
            # '백병원에는피부과' 로 주워 오면 버린 판단이 뒤집힌다.
            # 앞 어절에 병원 꼬리가 있으면 **그 어절만 버린다.**
            # "병원은 한마음 정형외과" 의 '병원은' 은 상호의 일부가 아니라
            # 앞말이다. 통째로 버리면 '한마음정형외과' 를 잃는다.
            #
            # 남은 것이 없으면(= 이름 전체가 병원 꼬리를 품고 있으면) 버린다 —
            # "백병원에는 피부과" 는 1차가 부정 문맥이라 버린 것이라 2차가
            # 주워 오면 안 된다.
            words = [w for w in words if not any(k in w for k in _HOSPITAL_SUFFIX)]
            if not words:
                continue
            name = "".join(words)
            # 전체가 진료과 이름 자체면 상호가 아니다 — '정형외과' 가
            # '정형'+'외과' 로 쪼개진다. 신경외과·성형외과도 같다.
            if (name + suffix) in _DEPT_SUFFIX:
                continue
            # **마지막 어절 + 꼬리가 진료과면 상호가 아니다.**
            # "아파가지고 정형외과" 가 '아파가지고 정형'+'외과' 로 쪼개졌다 —
            # 붙어 있어서 띄어쓰기 검사도 건너뛴다. 꼬리가 진료과 낱말을
            # 완성하고 있을 뿐이면 그 앞은 상호가 아니다.
            if (words[-1] + suffix) in _DEPT_SUFFIX:
                continue
            # 용언 어미로 끝나면 상호가 아니다 — "가고 싶은데정형외과" 처럼
            # STT 가 띄어쓰기를 놓친 경우가 실제로 들어온다.
            if name.endswith(_VERB_ENDINGS):
                continue
            return name + suffix
        return None
