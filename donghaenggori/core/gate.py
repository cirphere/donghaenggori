...
_QUESTION_HINTS = {
    ...
    "dept": ("어느 과", "진료과"),
    # 이동지원도 확정을 막지 않는다(BLOCKING 에 없다). 통화에서만 되묻는다.
    "mobility_need": ("혼자", "같이 가", "도움"),
    ...
    # **모호한 날짜·시각 질문에는 그 낱말이 안 들어간다.** 어르신이 말한
    # 표현이 그대로 들어가기 때문이다 — "말씀하신 9월 5일 / 금요일 중에 어느
    # 쪽으로 잡을까요?" 에는 '날짜' 도 '언제' 도 없다. 그래서 question 이
    # None 이 되고, 화면은 차단만 보여주고 통화 후속질문은 날짜를 건너뛴다.
    # 병원에서 같은 일이 있었다(#117) — 상호가 들어가서 '병원' 이 없었다.
    #
    # 모호 질문의 고정 문구('어느 쪽으로', '중에')를 힌트에 넣는다.
    "date": ("날짜", "언제", "며칠", "어느 쪽으로", "잡을까요"),
    "time": ("시각", "시간", "오전", "오후", "몇 시", "어느 쪽으로", "잡을까요"),
