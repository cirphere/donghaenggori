...
class ConfirmIn(BaseModel):
    ...
    #
    # **'직접 응대함' 은 성격이 다르다.** 나머지 넷은 "확인을 못 했거나 안
    # 했다" 인데 이것은 "사람이 통화해서 처리했다" 다. 새 유형(requesttype)의
    # 정상 경로가 여기라서 넣는다 — 복지사가 전화를 걸어 무엇이 필요한지
    # 알아내고 접수하는 것이 그 유형의 처리인데, 그걸 '물어볼 필요 없음' 이나
    # '기타' 로 적으면 감사 로그가 사실과 달라진다. 확인 질문 유효율(파일1
    # 4-2)의 분모에서도 빠져야 하는 건이다 — 질문이 쓸모없었던 게 아니라
    # 사람이 답을 얻어낸 것이다.
    acknowledge_reason: Literal[
        "이미 알고 있음", "물어볼 필요 없음", "연락이 닿지 않음",
        "직접 응대함", "기타"
    ] | None = Field(
