...
로그인은 **아이디**(직원번호)로 한다. 이메일은 연락처일 뿐이라 비워도 된다.
...
def run() -> None:
    db.init_db()
    user_id = input("아이디 (로그인에 쓴다, 예: U001): ").strip()
    name = input("이름: ").strip()
    role = input(f"역할 {ROLES}: ").strip()
    if role not in ROLES:
        print(f"알 수 없는 역할: {role!r} — {ROLES} 중 하나여야 합니다")
        return
    # 이메일은 연락처일 뿐이라 비워도 된다. 로그인은 아이디로 한다.
    email = input("이메일 (선택, 그냥 Enter): ").strip() or None
    ...
    if len(pw1) < 8:
        ...
        return
    try:
        user = db.create_user(user_id, name, role, pw1, email)
    except ValueError as e:
        print(f"실패: {e}")
        return
    print(f"완료: {user['id']} {user['name']} ({user['role']}) — 이 아이디로 로그인합니다")
