...
| `need_level` / `need_reasons` | 동행 **지원 수준** — 프로필(장기요양등급·거동) 기반 |
| **`mobility_need`** | 이동지원 **필요 여부** — 이번 통화 발화 기반. 아래 참조 |
| **`guardian_mentioned`** | 통화에서 들은 보호자 이야기 — `guardian`(연락처)과 다르다 |
...
**한 칸에 합치면 안 된다.** 같은 이름처럼 보이지만 나온 곳이 다르다.

| 키 | 출처 | 무엇 |
|---|---|---|
| `need_level` `need_basis` `need_official` | **프로필** | 지원 수준 — 단순 안내 / 차량+동행 / 휠체어·부축 동행. 배차 판단에 쓴다 |
| `mobility_need` | **이번 통화 발화** | 필요 여부 — 어르신이 이 통화에서 뭐라고 했나 |
| `guardian` | **프로필** | 이름·관계·연락처. 못 만났을 때 전화할 곳 |
| `guardian_mentioned` | **이번 통화 발화** | 통화에서 들은 보호자 이야기 |
...
"mobility_need": {
  ...
  "상태": "확인됨",
  "근거문구": ["원문 '아들내미가 델러다'",
             "가족이 데려다주기로 함 — 우리 동행은 불필요하나, 가족이 못 오게 되면 바로 필요해진다"]
},
"guardian_mentioned": {"판정": "언급있음", "내용": "아들내미 이(가) 데려다줄 수 있음", "상태": "확인됨", "근거문구": […]}
```

- `판정`은 다섯이다 — `명시적_필요` `명시적_불필요` `가족지원있음` `암시적_필요` `신호없음`
- **`가족지원있음`을 `명시적_불필요`와 갈라 보여줄 것.** 둘 다 '불필요'지만 근거가 다르다 —
  가족이 못 오게 되면 **바로 필요해진다.** 어르신이 스스로 괜찮다고 한 것과 같은 배지로
  그리면 그 차이가 사라진다
- `신호없음`이면 `필요여부: null` · `상태: 확인 필요` 다. **이때 `need_level`(프로필 값)을
  이 칸에 끌어다 쓰지 말 것** — 그게 이 분리의 이유다
- 화면은 두 값을 나란히, 출처를 붙여 보여준다: **"지원 수준(프로필 기반)"** / **"이번 통화
  언급(발화 기반)"**. 이 저장소의 정적 콘솔이 그렇게 그리고 있다
- `fields.mobility_need` 로도 같은 값이 온다(항목 라벨 `이동지원(발화)`). **`fields.mobility`
  가 아니다** — 카드의 `mobility`(거동 상태, 프로필)와 겹치지 않게 이름을 갈랐다
- **`신호없음`이면 통화 중에 되묻는다** — "병원까지 혼자 가시기 어려우신가요?". 되묻는
  항목 여섯 중 하나다(병원·방문일·시각·진료과·이동지원·대상자). 다만 **확정을 막지는
  않는다** — 이동지원을 몰라도 동행은 나간다
