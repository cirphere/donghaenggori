...
`origin/main` 에 새 커밋이 올라오면 5분 안에 반영된다. cron 이 5분마다
`/home/circle/aix/auto-deploy.sh` 를 부른다.

**바뀐 것이 없으면 아무것도 하지 않는다.** 이 기기는 실제 전화(ClawOps
웹훅)를 받고 있어서, 무조건 5분마다 컨테이너를 내렸다 올리면 그때 걸려온
통화가 끊긴다. HEAD 를 비교해 다를 때만 재배포한다.

`down` 은 쓰지 않는다. `up -d --build` 가 바뀐 컨테이너만 다시 만들어
멈추는 시간이 훨씬 짧다.

안전장치 넷:

- **flock** — 빌드가 5분을 넘겨도 다음 cron 과 겹치지 않는다
- **워킹트리 검사** — 커밋 안 한 변경이 있으면 건너뛴다. 자동 배포가
  사람이 작업 중인 파일을 덮으면 안 된다
- **fast-forward 만** — 로컬이 갈라졌으면 멈추고 로그에 남긴다
- **헬스체크** — 뜬 것과 응답하는 것은 다르므로 `/api/health` 200 까지 본다

```bash
tail -f /home/circle/aix/auto-deploy.log   # 배포가 일어날 때만 기록된다
crontab -l                                  # 등록 확인
```

스크립트를 저장소 밖에 두는 이유가 있다 — 저장소 안에 두면 자기 자신을
pull 하는 도중에 bash 가 읽고 있는 파일이 바뀌어 깨질 수 있다.
