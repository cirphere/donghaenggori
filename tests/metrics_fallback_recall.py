...
def main() -> int:
    ...
    print("  " + "-" * 74)
    print("  짚어둘 것 — 대리 접수 1건")
    print("  " + "-" * 74)
    print("  보호자 번호로 걸려온 대리 접수는 대상자가 '확인 필요'가 아니라 '추정'이다.")
    print("  보호자 연락처에 등록된 어르신이 정확히 1명이고 근거도 남으므로, 게이트가")
    print("  추정을 막지 않는 설계상 의도대로 동작한 것이다.")
    print()
    print("  다만 표시 문자열과 상태가 어긋나 있다 —")
    print("    target        '박순자 (보호자 대리 요청 — 확인 필요)'")
    print("    fields.target  status='추정'")
    print("  화면 문구는 확인하라고 말하는데 게이트는 통과시킨다. 이 건을 빼면")
    print("  재현율이 20/20 이 되지만, 숫자를 맞추려고 케이스를 고르는 것이므로")
    print("  그대로 두고 미달로 센다.")
    print()
